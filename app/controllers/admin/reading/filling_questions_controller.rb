class Admin::Reading::FillingQuestionsController < ApplicationController
  layout "admin/application"
  before_action :find_question_type, except: %i(index new create)

  def new
    @question_type = QuestionType.new topic_id: Topic.codes[:fill]
    Settings.Admin.filling_question.num_question
            .times{@question_type.questions.build}
    @question_type.questions.each do |question|
      Settings.Admin.filling_question.num_answer
  ...
  def create
    ...
    @question_type.topic_id = Topic.codes[:fill]
